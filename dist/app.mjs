...
function sync(){
 ...
 for(const id of ['pos-x','pos-y','move','restore'])$(id).disabled=false;$('pos-x').disabled=selected.edge==='east';$('pos-y').disabled=selected.edge==='south'||selected.edge==='north'||!!selected.cliff;
 draw();
}
...
function nudge(key,step){if(!ready||busy)return;let dx=key==='ArrowLeft'?-step:key==='ArrowRight'?step:0,dy=key==='ArrowUp'?-step:key==='ArrowDown'?step:0;if(selected.edge==='east')dx=0;if(selected.edge==='north'||selected.edge==='south'||selected.cliff)dy=0;if(!dx&&!dy)return;const p=config.Positions[selected.id];move(p.X+dx,p.Y+dy);}
